...
class EvalCallback():
    def __init__(self, net, input_shape, num_classes, image_ids, dataset_path, log_dir, cuda, \
        ...
        self.log_dir = log_dir
        self.cuda = cuda
        self.miou_out_path = miou_out_path + '_' + self.dataset_path.split('/')[-2]
    ...
    def get_miou_png(self, image):
        ...
        #   添加上batch_size维度
        # ---------------------------------------------------------#
        image_data = np.expand_dims(np.transpose(preprocess_input(np.array(image_data, np.float32)), (2, 0, 1))[1], 0)
    ...
    def on_epoch_end(self, epoch, model_eval):
        if epoch % self.period == 0 and self.eval_flag:
            ...
            from .utils_metrics import PD_FA, mIoU
            PD_FA = PD_FA()
            mIoU = mIoU()
            _, IoUs, _, _ = compute_mIoU(gt_dir, pred_dir, self.image_ids, self.num_classes, PD_FA, mIoU, None)  # 执行计算mIoU的函数
